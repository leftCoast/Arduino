...
void UI::begin(void) {
   ...
   hookup();
   screen = (displayObj*) new adafruit_684_Obj(OLED_CS,OLED_RST);
   if (screen) {
      if (screen->begin()) {
   ...
   if (!mHaveScreen) {
      Serial.println("Sorry no screen. Halting process.");
      while(1) delay(10);
   }
...
// Updated and tested with the new code. But, no
// one has a call for this. Good exercise though.
void UI::sensorDeath(void) {

   rect     screenRect(0,0,96,64);
   bmpObj*  bombImage;
   
   crash = true;
   if (mHaveScreen) {
      bombImage = new bmpObj(&screenRect,"/bomb9664.bmp");
      viewList.addObj(bombImage);
      bombImage->draw();
   }
   pinMode(13, OUTPUT);
   while(crash) {
...
void UI::idle(void) {
  ...
  colorObj  aColor;
   ...
   if (mHaveScreen && ding()) {                                // If we have a screen and its time to check stuff..
      if (ourSleepMgr.checkSleep()!=screen->getPercBlack()) {  // If we need to change the dimming value of the display..
         screen->setPercBlack(ourSleepMgr.checkSleep());       // Set the new diming value.
      ...
      if (isLogging()==mLoggingInd->isHolding()) {       // If they match.. (Sadly don't know what I'm doing here.)
