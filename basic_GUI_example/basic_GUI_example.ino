...
// This is a basic starting point for a single screen LC_GUI application.
// It give you the a good starting point to how the LC_GUI system works. 
//
// You will need..
// LC_baseTools         - Basic building blocks for all of the code.
// LC_Adafruit_1947     - Glue code to allow the Adafruit 1947 touch screen to be used by LC_GUI system.
// LC_GUIbase           - Foudation mathematics for things ike points, rectangles etc.
// LC_GUIItems          - A grab bag of GUi items that can be used on the screen or used as bases for custom items.
// LC_GUITextTools      - A grab bag of GFUI items that manipulate text. Edit, display ect.
// LC_SPI               - Pin assignments and other SPI/processor specific info.
...
void setup() {
   ...
   if (!initScreen(ADAFRUIT_1947,ADA_1947_SHIELD_CS,PORTRAIT)) {  // If we can't initialze our display..
      ...
      Serial.flush();                                             // Teensy's don't block on Serial.print calls. Force it!
   ...
   aFlasher->setPeriod(250);                                      // We'll set the period at 250 Ms. This will be faster than the default.
   aFlasher->setPulse(150);                                       // We'll set the pulse to 125 Ms. Also faster than the default.
   ...
   aRect->setCallback(firstButton);                               // And we'll set a callback. Otherwise yuo can inherit the button and write in a custom action() function.
